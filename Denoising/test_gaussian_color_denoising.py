...
import sys
sys.path.insert(0, 'H:\SUTNet')

from basicsr.models.archs.new_sutnet_arch import NSUTNet
from basicsr.models.archs.sutnet_arch import SUTNet
...
parser = argparse.ArgumentParser(description='Gaussian Color Denoising using SUTNet')

parser.add_argument('--input_dir', default='./Datasets/test/', type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='./result/Gaussian_Color_Denoising/', type=str, help='Directory for results')
parser.add_argument('--weights', default='./pretrained_models/gaussian_color_denoising', type=str, help='Path to weights')
parser.add_argument('--model_type', choices=['non_blind','blind'], default='non_blind', type=str, help='blind: single model to handle various noise levels. non_blind: separate model for each noise level.')
...
if args.model_type == 'blind':
    yaml_file = 'Options/GaussianColorDenoising_SUTNet.yml'
else:
    yaml_file = f'Options/GaussianColorDenoising_SUTNetSigma{args.sigmas}.yml'
import yaml
...
for sigma_test in sigmas:
    ...
    model_restoration = SUTNet(**x['network_g'])
    if args.model_type == 'blind':
