...
def parse_options(is_train=True):
    ...
    parser.add_argument(
        '-opt', type=str, default='../Denoising\Options\ImageSR_SUTNet.yml',
...
def init_loggers(opt):
    ...
    logger.info(get_env_info())  # 输出版本信息， 可以省略
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if mini_gt_size < gt_size:
                x0 = int((gt_size - mini_gt_size) / up_scale * random.random())
                y0 = int((gt_size - mini_gt_size) / up_scale * random.random())
                x1 = int(x0 + mini_gt_size / up_scale)
                y1 = int(y0 + mini_gt_size / up_scale)
                lq = lq[:, :, x0:x1, y0:y1]
                gt = gt[:, :, x0 * up_scale:x1 * up_scale, y0 * up_scale:y1 * up_scale]
            ###-------------------------------------------
